...
for k = 1:n-1
  ...
  if th(4,k+1) > TintSP(k+1) %only heating
    G(12,12) = 0; u(k+1,6) = 1/3*taugSW*epswSW*Sg*PhiDiff(k+1);
  else
    G(12,12) = Kp; u(k+1,6) = taugSW*epswSW*Sg*PhiDiff(k+1);
  endif
  
  if (th(4,k+1) > Temp(k+1)) && (th(4,k+1) > TintSP(k+1))  %free cooling
    G(11,11) = Va/3600*rhoa*ca;
  else
    G(11,11) = 10*Va/3600*rhoa*ca;
  endif
  
  if th(4,k+1) > TintSP(k+1) % shading
    u(k+1,6) = 1/10*taugSW*epswSW*Sg*PhiDiff(k+1);
  else
    u(k+1,6) = taugSW*epswSW*Sg*PhiDiff(k+1);
  endif
...
hold on, plot(Time/3600, th(4,:),'r'), hold off

subplot(212), plot(Time/3600,Qhvac)
